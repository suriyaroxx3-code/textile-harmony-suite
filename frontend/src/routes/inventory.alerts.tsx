...
// Supplier / lead-time lookup for known materials (not stored in the API yet)
const supplierInfo = {
...
function Page() {
  ...
  useEffect(() => {
    // Items below their minimum threshold, straight from the API
    api
      .get("/api/stock/low")
      .then((items) =>
        setAlerts(
          items.map((item) => ({
            ...item,
            ...(supplierInfo[item.name] ?? { supplier: "TBD", eta: "Pending" }),
          })),
        ),
      )
      .catch(console.error);
