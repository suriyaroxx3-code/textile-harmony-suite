...
function Page() {
  ...
  const fetchRecords = () => {
    api.get("/api/billing").then(setRecords).catch(console.error);
  };
  ...
  // `ref` is the human-readable order id (Q-0105 / INV-...); `id` is the DB key.
  ...
  const handleAdd = async () => {
    if (!newEntry.ref || !newEntry.contractor) return;
    try {
      await api.post("/api/billing", { ...newEntry, value: Number(newEntry.value), type: "quote" });
      fetchRecords();
      setIsAdding(false);
      setNewEntry({ ref: "", contractor: "", date: "", value: "", status: "Draft" });
    } catch (err) {
      alert(`Could not add quotation: ${err.message}`);
    }
  };

  const handleEdit = async () => {
    if (!editEntry.ref || !editEntry.contractor) return;
    try {
      await api.put(`/api/billing/${editingId}`, {
        ref: editEntry.ref,
        contractor: editEntry.contractor,
        date: editEntry.date,
        value: Number(editEntry.value),
        status: editEntry.status,
      });
      fetchRecords();
      setEditingId(null);
      setEditEntry({ ref: "", contractor: "", date: "", value: "", status: "" });
    } catch (err) {
            ...
            <tbody>
              ...
              {isAdding && (
                <tr className="bg-secondary/20">
                  <td className="px-6 py-2"><input className={inputCls} placeholder="e.g. Q-0105" onChange={e => setNewEntry({...newEntry, ref: e.target.value})} /></td>
                ...
                .map((r) => (
                  editingId === r.id ? (
                    <tr key={r.id} className="bg-secondary/20">
                      <td className="px-6 py-2"><input className={inputCls} value={editEntry.ref} onChange={e => setEditEntry({...editEntry, ref: e.target.value})} /></td>
                    ...
                    <tr key={r.id} className="border-b border-border/60 last:border-0 hover:bg-secondary/30 transition">
                      <td className="px-6 py-3 font-medium">{r.ref}</td>
