...
// Formats 384000 -> "₹3.8L" the way the stat card expects
...
function Dashboard() {
  ...
  useEffect(() => {
    api.get("/api/reports/dashboard").then(setSummary).catch(console.error);
  }, []);

  const trend = (summary?.trend ?? []).map((p) => ({ d: p.d, v: p.packed }));
  const kpis = [
    {
      label: "Units Packed Today",
      value: summary ? summary.units_packed_today.toLocaleString() : "—",
      trend: "Live from API",
      tone: "text-emerald-600",
    },
    {
      label: "Workers Present",
      value: summary ? `${summary.workers_present} / ${summary.workers_total}` : "—",
      trend: "Marked today",
      tone: "text-emerald-600",
    },
    {
      label: "Pending Bills",
      value: summary ? lakhs(summary.pending_bills_value) : "—",
      trend: summary ? `${summary.pending_bills_count} invoices` : "",
