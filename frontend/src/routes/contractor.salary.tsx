...
function Page() {
  ...
  const fetchRows = () => {
    api.get("/api/contractors").then(setRows).catch(console.error);
  };
  ...
  // "Pay now" -> mark the contractor as Paid on the server, then refresh
  const handlePay = async (r) => {
    if (r.status === "Paid") return;
    if (!confirm(`Mark ₹${r.amount.toLocaleString("en-IN")} as paid to ${r.name}?`)) return;
    await api.put(`/api/contractors/${r.id}`, { status: "Paid" });
    fetchRows();
  };
              ...
              {filteredRows.map((r) => (
                <tr key={r.id} className="border-b border-border/60 last:border-0 hover:bg-secondary/30 transition">
                  ...
                  <td className="px-6 py-3 text-right">
                    <Btn variant={r.status === "Paid" ? "ghost" : "primary"} onClick={() => handlePay(r)}>
